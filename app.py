...
# --- Pasta dos GeoPackages por UF ---
PASTA_GPKG = r"D:\VSCode\FUNASA\Censo-2022-Analise_Geoespacial\Setores_UF"

# Mapeamento do Código do IBGE para a Sigla da UF
COD_UF_PARA_SIGLA = {
...
def ler_uf(cd_uf, cd_mun=None, cd_dist=None, cd_subdist=None):
    sigla_uf = COD_UF_PARA_SIGLA.get(cd_uf)
    if not sigla_uf:
        return gpd.GeoDataFrame() # Retorna vazio se não achar a UF
        
    # Monta o caminho dinâmico: D:\...\Setores_UF\AC_setores_CD2022.gpkg
    caminho_gpkg = os.path.join(PASTA_GPKG, f"{sigla_uf}_setores_CD2022.gpkg")
    ...
    return gpd.read_file(caminho_gpkg, where=where)
...
def atualizar_municipios(cd_uf):
    ...
    gdf_uf = gpd.read_file(
        caminho_gpkg, where=f"CD_UF = '{cd_uf}'", columns=["CD_MUN", "NM_MUN"]
    )
    df = gdf_uf[["CD_MUN", "NM_MUN"]].drop_duplicates().sort_values("NM_MUN")
    return [
        {"label": r["NM_MUN"], "value": r["CD_MUN"]} for _, r in df.iterrows()
    ], None
...
def atualizar_distritos(cd_uf, cd_mun):
    ...
    gdf_f = gpd.read_file(caminho_gpkg, where=where, columns=["CD_DIST", "NM_DIST"])
    df = gdf_f[["CD_DIST", "NM_DIST"]].drop_duplicates().sort_values("NM_DIST")
    if df.empty:
        return [], None
    return [
        {"label": r["NM_DIST"], "value": r["CD_DIST"]} for _, r in df.iterrows()
    ], None
...
def atualizar_subdistritos(cd_uf, cd_mun, cd_dist):
    ...
    gdf_f = gpd.read_file(caminho_gpkg, where=where, columns=["CD_SUBDIST", "NM_SUBDIST"])
...
def atualizar_mapa(cd_uf, cd_mun, cd_dist, cd_subdist, cd_sit):
    ...
    if cd_sit == "urbana":
        ...
        gdf_filtrado = gdf_filtrado[
            gdf_filtrado["CD_SIT"].isin(["5", "6", "7", "8", "9"])
        ]
